...
function UpdateBackground() {
    ...
    $("body").css("background-image", "url('/wp-content/themes/astral-wp/library/images/background/" + a + ".jpg')");
}

function RouteProject(a) {
    UpdateBackground(), appVM.projectVM.slug(a), $menu.find(".menuItem:first-child").css({
...
function RouteHome() {
    UpdateBackground(), appVM.projectVM.slug(null), console.log("home"), $menu.find(".menuItem:first-child").css({
        "background-color": "black",
        color: "white"
    }), $projectNode.style.display = "none", $homeNode.style.display = "block";
}

function RouteHomeToAnchor(a) {
    UpdateBackground(), appVM.projectVM.slug(null), $menu.find(".menuItem:first-child").css({
        "background-color": "black",
        color: "white"
    }), $projectNode.style.display = "none", $homeNode.style.display = "block", foo = a, 
    console.log(a), console.log("home-anchor"), void 0 !== a && ScrollToAnchor(a);
}

function Project(a, b, c, d, e, f, g) {
    var h = this;
    h.id = a, h.slug = b, h.name = c, h.description = d, h.tags = e, h.featureMedia = f, 
    h.featureMediaUrl = ko.observable(), h.media = g;
}
...
function Media(a, b, c) {
    var d = this;
    d.content = a, d.description = b, d.type = c;
}
...
function ProjectListViewModel(a) {
    ...
    jQuery.getJSON(serviceRoot + "/astral/v1/project", null, function(b) {
        c.projects(jQuery.map(b, function(b, d) {
            var e = jQuery.map(b.tags, function(b) {
                return a.tagListVM.tagLookup[b.id];
            });
            return c.projectLookup.push(b.slug), new Project(b.id, b.slug, b.name, null, e, b.featureMedia, null);
        ...
        if (a.tagListVM.selectedTags() && 0 !== a.tagListVM.selectedTags().length) {
            ...
            c.selectedProjects(SplitArrayIntoN(d, b));
        } else c.projectLookup([]), c.projects().forEach(function(a, b) {
            c.projectLookup.push(a.slug);
        }), c.selectedProjects(SplitArrayIntoN(c.projects(), b));
    };
}
    ...
    ko.computed(function() {
        ...
        !1) : (jQuery.getJSON(serviceRoot + "/astral/v1/project/" + d, null, function(e) {
            var f = jQuery.map(e.tags, function(b) {
                ...
                return new Media(a.content, a.description, a.type);
            });
...
var $projectNode = document.getElementById("project"), $homeNode = document.getElementById("home"), $menu = jQuery(".menu"), $project = jQuery(".project"), $topAnchor = jQuery(".topAnchor"), $projectImage = jQuery(".projectImage"), $astralBanner = jQuery(".astralBanner"), serviceRoot = "http://192.168.99.100:8081/wp-json", appVM = new AppViewModel();

$(function() {
    ko.applyBindings(appVM, document.getElementById("content")), UpdateBackground(), 
    DisplayScrollToTop(), $(window).on("scroll", function() {
        DisplayScrollToTop();
