...
function ScrollToTop() {
    var a = previousRoute.split("/")[1];
    return $("html, body").animate({
        scrollTop: 0
    }, 500), "home" === a && (window.location.hash = "/home"), !1;
}

function ScrollToAnchor(a, b) {
    null === b && (b = 500);
    var c = $("#" + a), d = -3;
    return "fixed" === $menu.css("position") && (d += $menu.height()), "fixed" === $astralBanner.css("position") && (d += $astralBanner.height()), 
    null !== $wpAdminBar && (d += $wpAdminBar.height()), $("html, body").animate({
...
function RouteHome(a) {
    var b = previousRoute.split("/")[1];
    UpdateBackground(), appVM.projectVM.slug(null), RecordPreviousRoute(), $menu.find(".menuItem:first-child").css({
        "background-color": "black",
        color: "white"
    }), $projectNode.style.display = "none", $homeNode.style.display = "block", null !== a && "" !== a && void 0 !== a && ScrollToAnchor(a, "home" === b ? 500 : 0);
}
...
function ProjectListViewModel(a) {
    ...
    c.selectedTagsSummary = ko.observable(), jQuery.getJSON(serviceRoot + "/astral/v1/project", null, function(b) {
        var d = [];
        c.projects(jQuery.map(b, function(b, c) {
            var e = jQuery.map(b.tags, function(b) {
                return a.tagListVM.tagLookup[b.id];
            });
            return d.push(b.slug), new Project(b.id, b.slug, b.name, null, e, b.featureImage[0], null);
        })), c.projectLookup(d), c.updateSelectedProjectList();
    }), c.updateSelectedProjectList = function() {
        if (a.tagListVM.selectedTags() && 0 !== a.tagListVM.selectedTags().length) {
            c.projectLookup([]);
            var d = [], e = ko.utils.arrayFilter(c.projects(), function(b) {
                var c = !1;
                return b.tags.forEach(function(e) {
                    -1 !== a.tagListVM.selectedTags().indexOf(e) && (d.push(b.slug), c = !0);
                }), c;
            });
            c.projectLookup(d), c.selectedProjects(SplitArrayIntoN(e, b)), c.selectedTagsSummary(c.updateSelectedTagsSummary());
        } else {
            c.projectLookup([]);
            var d = [];
            c.projects().forEach(function(a, b) {
                d.push(a.slug);
            }), c.projectLookup(d), c.selectedProjects(SplitArrayIntoN(c.projects(), b)), c.selectedTagsSummary("");
        }
    ...
    ko.computed(function() {
        ...
        !1) : (jQuery.getJSON(serviceRoot + "/astral/v1/project/" + d, null, function(e) {
            var f = jQuery.map(e.tags, function(b) {
                ...
                return new Media(a.content, a.description, a.type, a.halfWidth);
            });
...
var $projectNode, $homeNode, $menu, $project, $topAnchor, $projectImage, $astralBanner, $wpAdminBar, $body, $backgroundImages, $tags, serviceRoot = "/wp-json", previousRoute = "", appVM = new AppViewModel(), backgroundImages = new Array();
...
$(function() {
    ...
    var a = window.location.hash, b = (window.location.href.split("#")[0], "/" === a.substr(1) || "" === a.substr(1) ? "home" : a.substr(1));
    router.setRoute(b);
});
